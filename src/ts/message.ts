...
export function buildMessageNode(fileName: string, exportMap: ExportMap, messageDescriptor: DescriptorProto, indentLevel: number, fileDescriptor: FileDescriptorProto): ts.ClassDeclaration {
    ...
    const classMembers: ReadonlyArray<ts.ClassElement> = [
        // ts.createMethodSignature(),
        // ts.createMethod(undefined, [ts.createToken(ts.SyntaxKind.AbstractKeyword)], undefined, "serializeBinary", undefined, [], [], ts.createTypeLiteralNode([]), undefined),
        // ts.createMethod
        // ts.createMethodSignature([], [], ts.createTypeLiteralNode([]), "serializeBinary", undefined),
        ts.createProperty(undefined, undefined, "serializeBinary()", undefined,
            ts.createTypeReferenceNode("Uint8Array", []), undefined),
        // ts.createMethodSignature()
        // ts.createProperty(undefined, undefined, "toObject")
    ];
    const inherits = [ts.createHeritageClause(ts.SyntaxKind.ExtendsKeyword, [
        ts.createExpressionWithTypeArguments([], ts.createIdentifier("jspb.Message"))
    ])]
    const exportModifier: ReadonlyArray<ts.Modifier> = [
        ts.createToken(ts.SyntaxKind.ExportKeyword),
    ];

    // const exp = ts.createClassExpression(
    //     exportModifier,
    //     messageName + "Test",
    //     undefined,
    //     inherits,
...
export function printMessage(fileName: string, exportMap: ExportMap, messageDescriptor: DescriptorProto, indentLevel: number, fileDescriptor: FileDescriptorProto, sourceFile: ts.SourceFile) {
    const messageName = messageDescriptor.getName();
    ...
    messageDescriptor.getNestedTypeList().forEach(nested => {
        const msgOutput = printMessage(fileName, exportMap, nested, indentLevel + 1, fileDescriptor, sourceFile);
        if (msgOutput !== "") {
    ...
    //     .printNode(ts.EmitHint.Unspecified, buildMessageNode(fileName, exportMap, messageDescriptor, indentLevel, fileDescriptor), sourceFile));
